# -*- coding: utf-8 -*-
"""
Created on Fri May  8 14:06:33 2020
...
wordnet = WordNetLemmatizer()  #wiser to lemmatize
sentences = nltk.sent_tokenize(paragraph)
corpus = []
for i in range(len(sentences)):
    review = re.sub('[^a-zA-Z]',' ',sentences[i])   #sub for replacing characters apart from A-Za-z with spaces  
    review = review.lower()  #lowering the words
    review = review.split()  #list of words
    #for stemming use ps object
    #review = [ps.stem(word) for word in review if not word in set(stopwords.words('english'))]  
    review = [wordnet.lemmatize(word) for word in review if not word in set(stopwords.words('english'))]  #for lemmatizing
    review = ' '.join(review)
